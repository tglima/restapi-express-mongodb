...
exports.validateSaveMessage = async (message) => {
  ...
  if (
    !validators.hasValue(message.deMessage) ||
    message.deMessage.trim().length < appConfig.qtMinCharMessage
  ) {
    returnValidate.messages.push(
      `Obrigatório informar uma mensagem com pelo menos ${appConfig.qtMinCharMessage} caracteres!`,
    );
    returnValidate.wasSuccess = false;
  }

  if (
    validators.hasValue(message.deEmail) &&
