...
const validateSaveOrder = async (order) => {
  ...
  if (
    ...
    (validators.isValidDate(order.dtStart) && order.dtStart < dateNow)
  ) {
  ...
  const resultFindCustomer = await customerModel.findByIdCustomer(
    order.idCustomer,
  );
  if (
    ...
    returnValidate.messages.push(
      'Não foi possível localizar o cliente informado!',
    );
    returnValidate.wasSuccess = false;
    return returnValidate;
  }
...
exports.validateSaveNewOrder = async (order) => {
  ...
  const resultFindOrder = await orderModel.findOrderByIdCustomer(
    order.idCustomer,
  );
